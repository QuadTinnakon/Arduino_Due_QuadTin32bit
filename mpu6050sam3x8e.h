...
float gyro[3];
float accel[3];
float gyroScaleFactor = radians(1000.0/32668.0);//32768.0 32730 , +-32756  32768.0
...
void Mag5883Read()
{
 ...
 int i = 0;
  ...
  Wire.endTransmission();   
  MagX1 = ((result[0] << 8) | result[1]);//offset + 1.05
  MagZ1 = ((result[2] << 8) | result[3])*-1;// + 0.05
  MagY1 = ((result[4] << 8) | result[5])*-1;// - 0.55
  MagXf = MagXf + (MagX1 - MagXf)*0.45;
...
void sensor_Calibrate()
{
  Serial.print("Sensor_Calibrate");Serial.println("\t");
    ...
    acc_offsetX = -0.13;//-0.18 0.11 -0.36  Trim PITCH CONTROL   -10.07	-10.55	-9.82
    acc_offsetY = -0.45;//0.16 -0.14 0.18 Trim ROLL CONTROL     10.39	9.74	11
